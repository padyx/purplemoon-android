...
public class IntegerListPreference extends DialogPreference {
	...
	public IntegerListPreference(Context context, AttributeSet attrs) {
		...
		// Retrieve our custom styleable xml attributes
		...
		if (mEntries == null) {
			throw new IllegalArgumentException("IntegerListPreference: Could not find entryList (strings) in xml");
		}

		int valuesResId = a.getResourceId(R.styleable.IntegerListPreference_valueList, 0);
		if (valuesResId == 0) {
			throw new IllegalArgumentException("IntegerListPreference: Could not find entryList (values[int]) in xml");
		}
	...
	protected void onPrepareDialogBuilder(Builder builder) {
		...
		if (mEntries == null || mValues == null) {
			throw new IllegalStateException(
					"Missing values: IntegerListPreference requires entryList[strings] and a valueList [ints]!");
		}
