 ...
 *
 */
public class PurplemoonAPIAdapter implements IPurplemoonAPIAdapter {
    ...
    public boolean doLogin(String username, String password) throws IOException, PurpleSkyException {
        PersistantModel model = PurpleSkyApplication.getContext().getPersistantModel();
        try {
    ...
    public List<OnlineBean> getOnlineFavorites() throws IOException, PurpleSkyException {
        ...
        if (response != null) {
            ...
            Map<String, MinimalUser> minimalUser = PurpleSkyApplication.getContext().getUserService()
    ...
    public MessageResult sendMessage(PrivateMessage message, SendOptions opts) throws IOException, PurpleSkyException {
        ...
        if (StringUtility.isNullOrEmpty(message.getMessageText())) {
            throw new PurpleSkyException(PurpleSkyApplication.getContext().getString(R.string.SendFailNoText));
        }
        ...
        if (result == null) {
            throw new PurpleSkyException(PurpleSkyApplication.getContext().getString(R.string.UnknownErrorOccured));
        }
    ...
    private List<PostIt> getPostits(AdapterOptions options, ArrayList<PostIt> list, StringBuilder urlBuilder) throws IOException, PurpleSkyException {
        ...
        if (userMap != null) { // Add to cache
            UserService service = PurpleSkyApplication.getContext().getUserService();
    ...
    public List<MinimalUser> searchUser(UserSearchOptions options) throws IOException, PurpleSkyException {
        ...
        JSONObject jsonObj;
        try {
            jsonObj = options.createSearchObject();
        } catch (JSONException e) {
    ...
    private String performGETRequestForString(URL resource) throws IOException, PurpleSkyException {
        try {
            ...
            if (resp.getResponseCode() >= 400) {
                ErrorTranslator.translateHttpError(PurpleSkyApplication.getContext(), resp.getResponseCode(), resp.getError(), resource.toString());
            }
    ...
    private HTTPURLResponseHolder performPOSTRequestForResponseHolder(URL resource, List<NameValuePair> postBody, List<NameValuePair> headrs)
        ...
        while (triesLeft > 0) {
            ...
            try {
                ...
                if (resp.getResponseCode() >= 400) {
                    ErrorTranslator.translateHttpError(PurpleSkyApplication.getContext(), resp.getResponseCode(), resp.getError(),
                            resource.toString());
                }
                return resp;
